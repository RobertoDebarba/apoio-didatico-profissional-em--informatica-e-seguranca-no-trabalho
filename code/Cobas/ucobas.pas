...
procedure TfrmCobas.FormShow(Sender: TObject);
var
...
begin
  ...
  Label3.Caption := cabecalho[0];

  cabecalho.Free;

  Application.CreateForm(TfrmTabela, frmTabela); // Cria form filho
end;

// Previsão de erros e outros ----------------------------------------------------------------------------------------

procedure TfrmCobas.Edit1Click(Sender: TObject);
begin
  if Edit1.Text = 'Digite o valor' then
    Edit1.Text := '';
end;

procedure TfrmCobas.Edit1KeyPress(Sender: TObject; var Key: char);
begin
  Key := UpCase(Key);
...
procedure TfrmCobas.EscreverHTMLdivisao(valor : integer; base : integer);
var
  arq : TextFile;
  x, y, i, resto : integer;
...
begin
  ...
  AssignFile(arq, 'Cobas/bin/index.html');
  ...
  IpHtmlPanel1.OpenURL(ExpandLocalHtmlFileName('Cobas/bin/index.html'));
end;

// Escrever HTML Tabela ----------------------------------------------------------------------------------------------

procedure TfrmCobas.EscreverHTMLtabela(valor : string; base : integer);
var
  arq : TextFile;
  x, y : integer;
...
procedure TfrmCobas.EscreverHTMLseparacao(valor : string; grupo : integer);
var
  arq : TextFile;
...
procedure TfrmCobas.EscreverHTMLdecomposisao(valor : string; base : integer);
var
...
procedure TfrmCobas.bttConverterClick(Sender: TObject);           // Converter
var
  valor : string;
  x : integer;
  i : char;
...
procedure TfrmCobas.bttBaseClick(Sender: TObject);                // Tabela
begin
  frmTabela.ShowModal;
end;
...
procedure TfrmCobas.FormClose(Sender: TObject; var CloseAction: TCloseAction);
begin
  frmTabela.Free;  // Mata form filho
